...
TRAIN_DIR = Path(os.path.expanduser("/Users/dilli/Desktop/Project_Arxiv/Preprocessing/processed_images/train"))
TEST_DIR = Path(os.path.expanduser("/Users/dilli/Desktop/Project_Arxiv/Preprocessing/processed_images/test"))
VAL_DIR = Path(os.path.expanduser("/Users/dilli/Desktop/Project_Arxiv/Preprocessing/processed_images/val"))
...
LR = 0.001
L2 = 1e-5
DROPOUT = 0.2  # Dropout rate for the final classifier
VARIANT = 's4'  # MobileOne variant to use
...
def create_mobileone_model(num_classes, checkpoint_path=None, variant='s4', device='cpu'):
    ...
    model.linear = nn.Sequential(
        nn.Dropout(p=DROPOUT),  # Add dropout here
        nn.Linear(in_features, num_classes)
    )
    return model

# Create model
model = create_mobileone_model(num_classes=num_classes, checkpoint_path=CHECKPOINT_PATH, variant=VARIANT, device=device)
...
N = 5  # You can increase this number to unfreeze more layers
params = list(model.named_parameters())
for i, (name, param) in enumerate(params):
    param.requires_grad = (i >= len(params) - N)

# Print unfrozen layers for confirmation
...
def train_model(model, train_dataloader, test_dataloader, optimizer, loss_fn, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        if test_loss < best_test_loss:
            ...
            torch.save(best_model_wts, MODEL_PATH / f"{best_test_loss:.5f}_best_model_by_test_loss.pth")
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(best_acc_model_wts, MODEL_PATH / f"{best_test_acc:.5f}_best_model_by_test_acc.pth")
...
if device.type == 'mps':
    ...
    print(f"Using MPS device, setting manual seed {seed_mps} for reproducibility.")
